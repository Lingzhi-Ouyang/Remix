...
InitializeIdTable(Remaining) == IF Remaining = {} THEN {}
                                ELSE LET chosen == CHOOSE i \in Remaining: TRUE
                                         re     == Remaining \ {chosen}
                                     IN {<<chosen, Cardinality(Server) - Cardinality(Remaining)>>} \union InitializeIdTable(re)
...
ElectLeader(i) ==
        ...
        /\ leaderOracle' = i
        /\ SwitchToLeading(i)
        /\ UNCHANGED <<servingState, acceptedEpoch, currentEpoch, lastCommitted, followerVars,
...
FollowLeader(i) ==
        ...
        /\ UNCHANGED <<servingState, acceptedEpoch, currentEpoch, lastCommitted, leaderAddr, electionVars,
...
FollowerProcessSyncMessage(i, j) ==
        ...
        /\ LET msg == rcvBuffer[j][i][1]
           IN UpdateRecorder(<<"FollowerProcessSyncMessage", i, j, msg.mtype, msg.mzxid>>)
        /\ UpdateAfterAction
        ...
        /\ LET msg    == rcvBuffer[j][i][1]
           ...
           IN /\ infoOk
              /\ \/ \* just reply UPTODATE.
                    ...
                    /\ UNCHANGED <<ackldRecv, zabState, servingState, lastCommitted, lastProcessed, daInv>>
                 \/ /\ ~QuorumFormed(i, sid_ackldRecv)
                    /\ \/ /\ match
                          ...
                          /\ LET new_sid_ackldRecv == {a.sid: a \in ackldRecv'[i]}
                             ...
                             \/ \* still wait in waitForNewLeaderAck.
                                /\ ~QuorumFormed(i, new_sid_ackldRecv)
                                /\ Discard(j, i)
                                /\ UNCHANGED <<zabState, servingState, lastCommitted, lastProcessed>>
                          /\ UNCHANGED daInv
                       \/ /\ ~match
